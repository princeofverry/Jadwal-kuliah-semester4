...
const Schedule = () => {
  ...
  return (
    <div className={`schedule-container mx-auto w-full md:px-48 px-4 ${darkMode ? 'dark' : ''} ${darkMode ? 'bg-gray-900' : 'bg-white'}`}>
      <h2 className={`text-3xl font-bold text-center py-4 ${darkMode ? 'text-[#D0D1D2]' : 'text-gray-500'}`}>Jadwal Kuliah</h2>
      <div className="justify-between flex md:flex-row flex-col">
        <p className={`text-lg mb-4 text-center ${darkMode ? 'text-[#D0D1D2]' : 'text-gray-500'}`}>Sekarang hari: {currentDay}, {currentDateFormatted}</p>
      ...
      {Object.keys(separatedSchedule).map(day => (
        <div key={day} className="pb-8">
          <h3 className={`text-2xl font-semibold mb-4 ${darkMode ? 'text-[#D0D1D2]' : 'text-gray-500'}`}>{day}</h3>
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            {separatedSchedule[day].map(item => (
              <div key={item.id} className={`bg-gray-100 hover:bg-gray-300 rounded p-4 flex items-center shadow ${darkMode ? 'dark:bg-[#1C1F23] dark:hover:bg-[#1b2b3b]' : ''}`}>
                <div className="flex-grow">
                  <div className={`font-semibold ${darkMode ? 'text-white' : 'text-gray-500'}`}>{item.course}</div>
                  <div className={`text-sm ${darkMode ? 'text-gray-300' : 'text-gray-500'}`}>{item.location}</div>
                  <div className={`text-sm ${darkMode ? 'text-gray-300' : 'text-gray-500'}`}>{item.time}</div>
                  <div className={`text-sm ${darkMode ? 'text-gray-300' : 'text-gray-500'}`}>Dosen:</div>
                  <ul className={`list-disc list-inside ${darkMode ? 'text-gray-300' : 'text-gray-500'}`}>
                    {item.lecturers.map((lecturer, index) => (
                      <li key={index} className={`text-sm ${darkMode ? 'text-gray-300' : 'text-gray-500'}`}>{lecturer}</li>
                    ))}
                  </ul>
                  <div className={`text-sm ${darkMode ? 'text-gray-300' : 'text-gray-500'}`}>{item.credits} SKS</div>
                </div>
              </div>
            ))}
          </div>
